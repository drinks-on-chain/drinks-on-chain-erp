...
// Los enlaces a las fichas de otros módulos (origen, vendimia, …) los construyen otras ramas; hasta
// integrarlas, la precarga de esas rutas responde 404 en esta rama.
const OTHER_MODULES = /^404 \/(origen|vendimia|vinificacion|crianza|destilacion)\//;
...
test("Altos: ninguna crianza liberada, el embotellado queda bloqueado por el candado", async ({ page }) => {
  const errors = trackErrors(page, [NO_LAB, OTHER_MODULES]);
...
test("Lotes: filtro por etapa y línea de tiempo del lote en reposo", async ({ page }) => {
  const errors = trackErrors(page, [OTHER_MODULES]);
  await login(page, "enologa@cintiviejo.test");
